...
if __name__ == "__main__":
    ...
    cfg = FoundationPoseWrapperConfig(
        ...
        est_refine_iter=3,  # Increase if the initial pose is not good enough
        track_refine_iter=2,  # Increase if the tracking is not good enough
        chunk_size=128,  # Specify which chunk size to use for the TensorRT engines (must match the value used in convert_onnx.sh when generating the engines
    )
